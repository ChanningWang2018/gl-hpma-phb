...
describe("DataService row index", () => {
  ...
  it("caches the index per trends object identity", () => {
    const trends = makeTrends();
    expect(DataService.getRowIndex(trends)).toBe(DataService.getRowIndex(trends));
  });
});
...
describe("DataService.extractData", () => {
  it("returns one entry per period with matched winrate/attendancerate", () => {
    ...
    expect(DataService.extractData(trends, 1, MODE_1V1_LOW)).toEqual([
      ...
      { period: "2026/01/17 - 2026/01/23", winrate: null, attendancerate: null },
    ]);
  });
  ...
  it("falls back to nulls for missing rows", () => {
    ...
    expect(DataService.extractData(trends, 2, MODE_2V2_HIGH)).toEqual([
      { period: "2026/01/03 - 2026/01/09", winrate: null, attendancerate: null },
      { period: "2026/01/10 - 2026/01/16", winrate: null, attendancerate: null },
      { period: "2026/01/17 - 2026/01/23", winrate: null, attendancerate: null },
    ]);
  });
  ...
  it("treats an unknown mode as missing data (all nulls), not a throw", () => {
    ...
    expect(result.every((r) => r.winrate === null && r.attendancerate === null)).toBe(true);
  });
...
describe("DataService.extractScatterData", () => {
  it("filters by period and mode, maps fields, and sorts by echoId ascending", () => {
    const trends = makeTrends();
    expect(DataService.extractScatterData(trends, "2026/01/03 - 2026/01/09", MODE_1V1_LOW)).toEqual([
      { reverberationid: 1, winrate: 52.0, attendancerate: 9.1, name: "Hermione Granger" },
      { reverberationid: 2, winrate: 55.01, attendancerate: 8.8, name: "Harry Potter" },
    ]);
    expect(DataService.extractScatterData(trends, "2026/01/10 - 2026/01/16", MODE_1V1_LOW)).toEqual([
      { reverberationid: 1, winrate: 41.2, attendancerate: 5.9, name: "Hermione Granger" },
      { reverberationid: 3, winrate: 40.44, attendancerate: 6.14, name: "Dobby" },
    ]);
  });
  ...
  it("labels unknown echo ids as Unknown Echo", () => {
    ...
    const result = DataService.extractScatterData(trends, "2026/01/17 - 2026/01/23", MODE_1V1_LOW);
    expect(result).toEqual([
      { reverberationid: 999, winrate: 10.0, attendancerate: 1.0, name: "Unknown Echo" },
    ]);
  });

  it("returns [] for unknown period, unknown mode, or null trends", () => {
    const trends = makeTrends();
    expect(DataService.extractScatterData(trends, "no-such-period", MODE_1V1_LOW)).toEqual([]);
    expect(DataService.extractScatterData(trends, "2026/01/03 - 2026/01/09", "no-such-mode")).toEqual([]);
    expect(DataService.extractScatterData(null, "2026/01/03 - 2026/01/09", MODE_1V1_LOW)).toEqual([]);
  });

  it("does not mutate the cached period/mode groups when sorting", () => {
    const trends = makeTrends();
    const first = DataService.extractScatterData(trends, "2026/01/03 - 2026/01/09", MODE_1V1_LOW);
    const { byPeriodMode } = DataService.getRowIndex(trends);
    ...
    const second = DataService.extractScatterData(trends, "2026/01/03 - 2026/01/09", MODE_1V1_LOW);
...
describe("DataService index rebuild on reload", () => {
  it("builds a fresh index for a new trends object and leaves the old one cached", () => {
    ...
    expect(DataService.extractData(first, 1, MODE_1V1_LOW)[0].winrate).toBe(52.0);
    ...
    expect(DataService.extractData(reloaded, 1, MODE_1V1_LOW)[0].winrate).toBe(77.7);
    ...
    expect(DataService.getRowIndex(first)).not.toBe(DataService.getRowIndex(reloaded));
  });
});
...
describe("DataService against the real archive", () => {
  ...
  it("matches a linear filter baseline for extractScatterData", () => {
    ...
    expect(result.map((r) => r.reverberationid)).toEqual(expected.map((r) => r[1]));
    expect(result.every((r, i) => r.winrate === expected[i][3] && r.attendancerate === expected[i][4])).toBe(true);
  });
});
