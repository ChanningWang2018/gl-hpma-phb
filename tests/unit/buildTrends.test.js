...
describe('DataService trends extractors', () => {
  ...
  it('extractScatterData matches the legacy archive-based output', () => {
    const scatter = DataService.extractScatterData(
      trendsShape,
      periodA,
      '1v1-low',
    );
  ...
  it('extractScatterData returns [] for unknown period/mode and handles empty trends', () => {
    expect(
      DataService.extractScatterData(trendsShape, 'nope', '1v1-low'),
    ).toEqual([]);
    expect(
      DataService.extractScatterData(trendsShape, periodA, '2v2-high'),
    ).toEqual([]);
